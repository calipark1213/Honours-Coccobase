...
```{r}
orig <- ggplot(data = data.full, mapping = aes(exp(cells_per_litre))) + geom_histogram(bins = 20) + xlab("cells per litre")
```

Log transformed: 
```{r}
log <- ggplot(data = data.full, mapping = aes(cells_per_litre)) + geom_histogram(bins = 20) + xlab("log cells per litre") 
```
...
```{r}
p1 <- grid.arrange(arrangeGrob(orig, log, nrow = 1))
