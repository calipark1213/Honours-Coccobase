...
```{r}
orig <- ggplot(data = data, mapping = aes(exp(cells_per_litre))) + geom_histogram(bins = 20) + theme(axis.title.x = element_blank())
```

Log transformed: 
```{r}
log <- ggplot(data = data, mapping = aes(cells_per_litre)) + geom_histogram(bins = 20) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) 
```
...
```{r}
dist<- ggplot(data = data) + coord_equal() + geom_point(aes(x = longitude, y = latitude, color = scientific_name)) + geom_map(data = WorldData, map = WorldData, aes(x = long, y = lat, map_id = region), fill = "white", colour = "#7f7f7f", alpha = 0.5, size=0.5) + theme(legend.position = "none") + ylim(40,90)  + xlim(-50,0)
